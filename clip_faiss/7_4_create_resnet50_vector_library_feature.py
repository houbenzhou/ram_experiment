...
def resize(image_path, image_size=(256, 256)):
    ...
    return img
#创建颜色直方图
def get_resnet50_features(image_path):
    """

    :return:
    """
    image = Image.open(image_path).convert('RGB')
    image_features = resnet_model.embeding(image)
...
if __name__ == "__main__":
    ...
    if id_type == 'category_name':
        out_path = os.path.join(base_path, 'output', 'faiss_model', 'resnet50_feature',out_name)
    ...
    for file_path in tqdm(file_paths, total=len(file_paths)):
        # 使用PIL打开图片
        image_features = get_resnet50_features(file_path)
