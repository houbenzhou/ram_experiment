...
# 保存为 JSON 文件
with open('./output/category_name.json', 'r') as json_file:
    id2filename = json.load(json_file)
index = faiss.read_index("./output/image_faiss.index")
...
if __name__ == "__main__":
    ...
    out_logs = os.path.join(base_images,"output","新零售图片数据_Trax_部分.txt")
    # 打印查询结果
        ...
        for img_path in img_paths:
            ...
            for i in range(len(filenames)):
                if filenames[0][0] == filenames[0][1]:
                    ...
                    f.write( f"img_path:{img_path}, true_name: {filenames[0][0]} ,pred_name: {filenames[0][1]}."+"\n")
                # print("img_path:", img_path, "true_name:", filenames[0][0], "pred_name:", filenames[0][1])
        f.write(f"correct_count:{correct_count}, total_count: {total_count} ,accuracy: {correct_count/total_count}."+"\n")
        # f.write(str("correct_count:",correct_count,"total_count:",total_count,"accuracy:",correct_count/total_count))
