...
if __name__ == "__main__":
    ...
    d = 2048
    # 输出文件名
    out_name = '35_category_05_normalized'
    ...
    if id_type == 'category_name':
        out_path = os.path.join(base_path, 'output', 'faiss_model', 'dinov2_clip_color_feature',out_name)
    ...
    for file_path in tqdm(file_paths, total=len(file_paths)):
        # 使用PIL打开图片
        image_features_color = get_image_color_features(file_path)
        image_features_clip = get_image_clip_features(file_path)
        image_features_dinov2 = get_dinov2_features(file_path)
        image_features=np.concatenate((image_features_dinov2,image_features_clip, image_features_color),axis=1)
